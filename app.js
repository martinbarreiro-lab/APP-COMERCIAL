...
function actualizarAvatar(perfil) {
  ...
  if (av && perfil) av.textContent = perfil.nombre?.substring(0, 2).toUpperCase() || 'US';
}
...
function mostrarApp() {
  ...
  const rol = estadoApp.perfil?.rol;
  if (rol === 'cliente') {
...
function navegarA(pagina) {
  // Manejo especial para resumen-pedido
  ...
  const titulos = {
    'dashboard':'La Cabaña','pedidos':'Pedidos','nuevo-pedido':'Nuevo pedido',
    'resumen-pedido':'Tu pedido','clientes':'Clientes','detalle-cliente':'Cliente',
    'form-cliente':'Nuevo cliente','cobros':'Cobros','nuevo-cobro':'Registrar cobro',
    'facturas':'Facturas y remitos','nueva-factura':'Nuevo documento',
    'entrega-dia':'Entrega del día','reclamos':'Reclamos','productos':'Productos',
    'reportes':'Reportes','rendicion':'Rendición','comisiones':'Mis comisiones',
    'hoja-ruta':'Hoja de ruta','mas':'Menú','config':'Configuración',
    'actualizar-precios':'Lista de precios','portal-cliente':'Mi cuenta',
  };
  ...
  const navMap = { 'dashboard':'dashboard','pedidos':'pedidos','nuevo-pedido':'pedidos','resumen-pedido':'pedidos','clientes':'clientes','detalle-cliente':'clientes','form-cliente':'clientes','cobros':'cobros','nuevo-cobro':'cobros','mas':'mas','facturas':'mas','nueva-factura':'mas','reclamos':'mas','productos':'mas','reportes':'mas','rendicion':'mas','comisiones':'mas','config':'mas','actualizar-precios':'mas' };
  const navKey = navMap[pagina];
...
async function cargarDashboard() {
  ...
  const { data: pedidosMes } = await sb.from('pedidos')
    .select('id, total, estado, fecha_pedido, numero, cliente_id, clientes(razon_social)')
  ...
  const { data: pedidosSinCobrar } = await sb.from('pedidos')
    .select('id, total, numero, cliente_id, clientes(razon_social)').eq('estado', 'entregado');
  ...
  if (estadoApp.perfil?.rol === 'admin') {
    const { data: comisionesMes } = await sb.from('comisiones').select('monto_comision').gte('created_at', inicioMes);
    const { data: comisionesAnio } = await sb.from('comisiones').select('monto_comision').gte('created_at', inicioAnio);
    document.getElementById('kpi-comisiones-mes').textContent = formatMoney(comisionesMes?.reduce((s,c) => s + c.monto_comision, 0) || 0);
    document.getElementById('kpi-comisiones-anio').textContent = formatMoney(comisionesAnio?.reduce((s,c) => s + c.monto_comision, 0) || 0);
...
async function cargarAlertas(pedidosSinCobrar) {
  ...
  pedidosSinCobrar?.forEach(p => {
    alertas.push({ tipo: 'danger', icon: 'ti-alert-circle', titulo: `${p.clientes?.razon_social} — pendiente de cobro`, sub: 'Registrar cobro →' });
  });

  const hoy = new Date();
  const inicioMes = new Date(hoy.getFullYear(), hoy.getMonth(), 1).toISOString().split('T')[0];
  const { data: clientesObjetivo } = await sb.from('clientes').select('razon_social, objetivo_kg_mensual, id').not('objetivo_kg_mensual', 'is', null);

  for (const c of clientesObjetivo || []) {
    const { data: pedidosCli } = await sb.from('pedidos').select('id').eq('cliente_id', c.id).gte('fecha_pedido', inicioMes).neq('estado', 'cancelado');
    if (!pedidosCli?.length) continue;
    const pedidoIds = pedidosCli.map(p => p.id);
    const { data: items } = await sb.from('pedido_items').select('cantidad').in('pedido_id', pedidoIds);
    const kgMes = items?.reduce((s, i) => s + (i.cantidad || 0), 0) || 0;
    if (kgMes / c.objetivo_kg_mensual < 0.7) {
  ...
  `).join('') : '<p style="font-size:13px;color:#aaa;text-align:center;padding:12px;">Sin alertas por ahora ✓</p>';
}
...
function renderUltimosPedidos(pedidos) {
    ...
    <div class="pedido-card" onclick="abrirPedido('${p.id}')">
      <div class="pedido-card-header">
        <div>
          ...
          <p class="pedido-card-num">#${String(p.numero||0).padStart(4,'0')} · ${formatFecha(p.fecha_pedido)}</p>
        </div>
  ...
  `).join('') : '<p style="font-size:13px;color:#aaa;text-align:center;padding:12px;">No hay pedidos este mes</p>';
}
...
function renderPedidos(pedidos) {
  document.getElementById('pedidos-container').innerHTML = pedidos.length ? pedidos.map(p => `
    <div class="pedido-card" onclick="abrirPedido('${p.id}')">
      <div class="pedido-card-header">
        <div>
          <p class="pedido-card-nombre">${p.clientes?.razon_social || 'Cliente'}</p>
  ...
  `).join('') : '<p style="font-size:13px;color:#aaa;text-align:center;padding:20px;">No hay pedidos</p>';
}
...
async function confirmarEstadoPedido(id, nuevoEstado) {
  await sb.from('pedidos').update({ estado: nuevoEstado, updated_at: new Date().toISOString() }).eq('id', id);
...
async function cargarCatalogo() {
  ...
  sel.innerHTML = '<option value="">Seleccioná un cliente...</option>' + (clientes||[]).map(c =>
    `<option value="${c.id}" data-beneficio="${c.tipo_beneficio}" data-pct="${c.beneficio_pct}" data-iva="${c.tipo_iva}">${c.razon_social}</option>`
  ).join('');
...
function onClienteSeleccionado() {
  ...
  let badge = beneficio === 'descuento_pct' ? `Desc. ${pct}%` : beneficio === 'bonificacion_kg' ? `Bonif. ${pct}% kg` : 'Sin beneficio';
  badge += ` · ${ivaLabels[iva]||''}`;

  document.getElementById('cliente-beneficio-badge').textContent = badge;
  document.getElementById('cliente-beneficio-badge').className = 'info-box blue';
  ...
  estadoApp.pedidoActual.clientePct = pct;
        ...
        <div class="producto-row-header">
          <div class="producto-row-info">
            ...
            <p class="producto-row-precio">
              ...
              ${tieneDesc && p.tiene_precio ? `<span class="producto-row-precio-tachado">${formatMoney(p.precio_lista)}</span>` : ''}
            </p>
          </div>
...
function cargarResumenPedido() {
  ...
  Object.keys(pedidoCantidades).forEach(id => {
    if (pedidoCantidades[id] > 0) {
      ...
      const sub = cantidad * prod.precio_lista;
      subtotalLista += sub;
      itemsRender.push({ prod, cantidad, cantBonif, sub });
    }
  });

  if (beneficio === 'descuento_pct') { descuentoMonto = subtotalLista*pct/100; subtotalNeto = subtotalLista - descuentoMonto; }
  else subtotalNeto = subtotalLista;
  ...
  document.getElementById('resumen-items-container').innerHTML = `
    <div class="card">
      ...
      ${itemsRender.map(i => `
        <div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:0.5px solid #f0ede6;">
          <div>
            <p style="font-size:13px;font-weight:500;">${i.prod.nombre}</p>
            <p style="font-size:10px;color:#aaa;">${i.cantidad} ${i.prod.unidad_venta==='caja'?'caja(s)':'unid.'}${i.cantBonif>0?` + ${i.cantBonif.toFixed(1)} bonif.`:''}</p>
          </div>
  ...
  document.getElementById('resumen-calculos').innerHTML = `
    ...
    ${descuentoMonto>0?`<div style="display:flex;justify-content:space-between;margin-bottom:5px;"><span style="font-size:12px;color:#3B6D11;">Desc. ${pct}%</span><span style="font-size:12px;color:#3B6D11;">− ${formatMoney(descuentoMonto)}</span></div>`:''}
    ${beneficio==='bonificacion_kg'?`<div style="display:flex;justify-content:space-between;margin-bottom:5px;"><span style="font-size:12px;color:#3B6D11;">Bonif. ${pct}% kg</span><span style="font-size:12px;color:#3B6D11;">incluida</span></div>`:''}
    ${ivaMonto>0?`<div style="background:#e6f1fb;border-radius:8px;padding:8px 10px;margin-bottom:8px;"><p style="font-size:11px;color:#185FA5;font-weight:500;">IVA ${iva==='mixto'?'mixto (10,5% ef.)':'21% completo'}</p><div style="display:flex;justify-content:space-between;margin-top:4px;"><span style="font-size:11px;color:#185FA5;">IVA</span><span style="font-size:11px;color:#185FA5;">${formatMoney(ivaMonto)}</span></div></div>`:''}
...
async function confirmarPedido() {
  ...
  const { data: pedido, error } = await sb.from('pedidos').insert({
    ...
    estado: 'pendiente',
    subtotal_lista: estadoApp.pedidoActual.subtotalLista||0,
    descuento_monto: estadoApp.pedidoActual.descuentoMonto||0,
    subtotal_neto: estadoApp.pedidoActual.subtotalNeto||0,
    iva_monto: estadoApp.pedidoActual.ivaMonto||0,
    total: estadoApp.pedidoActual.total||0,
...
function enviarPresupuestoWhatsapp() {
  ...
  const itemsTxt = Object.keys(pedidoCantidades).filter(id=>pedidoCantidades[id]>0).map(id => {
    const prod = catalogoProductos.find(p=>p.id===id);
...
async function renderClientes(clientes) {
  ...
  const html = await Promise.all(clientes.map(async c => {
    const { data:pedsCli } = await sb.from('pedidos').select('id').eq('cliente_id',c.id).gte('fecha_pedido',inicioMes).neq('estado','cancelado');
    let kgMes = 0;
    if (pedsCli?.length) {
      const { data:items } = await sb.from('pedido_items').select('cantidad').in('pedido_id',pedsCli.map(p=>p.id));
      kgMes = items?.reduce((s,i)=>s+(i.cantidad||0),0)||0;
    }
    ...
    if (c.objetivo_kg_mensual) {
      ...
      barraHtml = `<div class="objetivo-bar"><div class="objetivo-bar-header"><span class="objetivo-bar-label">Obj: ${c.objetivo_kg_mensual.toLocaleString()} kg/mes</span><span class="objetivo-bar-valor ${clase}">${label}</span></div><div class="progress-bar"><div class="progress-fill ${clase}" style="width:${pct}%;"></div></div></div>`;
    } else {
      barraHtml = `<div class="objetivo-bar"><span style="font-size:10px;color:#aaa;">Sin objetivo · ${Math.round(kgMes).toLocaleString()} kg este mes</span></div>`;
    }
    return `
      <div class="cliente-card" onclick="abrirCliente('${c.id}')">
        <div class="cliente-card-header">
          ...
          <div class="cliente-card-info">
            ...
            <p class="cliente-card-sub">${beneficioLabel(c)}</p>
          </div>
...
async function abrirCliente(id) {
  ...
  await cargarDetalleCliente(c);
}

async function cargarDetalleCliente(c) {
  const hoy = new Date();
  const inicioMes = new Date(hoy.getFullYear(),hoy.getMonth(),1).toISOString().split('T')[0];
  const { data:pedidos } = await sb.from('pedidos').select('id, total, estado, fecha_pedido, numero').eq('cliente_id',c.id).neq('estado','cancelado');
  const { data:cobros } = await sb.from('cobros').select('monto').eq('cliente_id',c.id);
  const pedIds = pedidos?.map(p=>p.id)||[];
  let kgMes = 0;
  if (pedIds.length) {
    const pedMes = pedidos?.filter(p=>p.fecha_pedido>=inicioMes).map(p=>p.id)||[];
    if (pedMes.length) {
      const { data:items } = await sb.from('pedido_items').select('cantidad').in('pedido_id',pedMes);
      kgMes = items?.reduce((s,i)=>s+(i.cantidad||0),0)||0;
    }
  }
  ...
  if (c.objetivo_kg_mensual) {
    ...
    document.getElementById('cli-objetivo-section').innerHTML = `
      <div class="objetivo-section">
        <div class="objetivo-header"><span class="objetivo-label">${c.objetivo_kg_mensual.toLocaleString()} kg / mes</span><span class="objetivo-valor ${clase}">${Math.round(kgMes).toLocaleString()} kg</span></div>
        <div class="progress-bar" style="height:8px;"><div class="progress-fill ${clase}" style="width:${pct}%;"></div></div>
        <p class="objetivo-msg" style="color:${colores[clase]};">${msgs[clase]}</p>
      </div>`;
  } else {
    document.getElementById('cli-objetivo-section').innerHTML='';
  }
  ...
  document.getElementById('cli-ultimo-pedido').innerHTML = ultimo ? `
  ...
  // Tab datos
  document.getElementById('datos-cliente-form').innerHTML = `
    <div class="card" style="padding:8px 14px;margin-bottom:8px;">
      ...
      <div style="display:flex;align-items:center;padding:8px 0;border-bottom:0.5px solid #f0ede6;"><span style="font-size:12px;color:#aaa;width:100px;">WhatsApp</span><span style="font-size:13px;color:#534AB7;">${c.whatsapp||c.telefono||'—'}</span></div>
    ...
    ${c.notas_internas?`<div class="card"><p class="section-title-sm">Notas internas</p><p style="font-size:12px;color:#555;line-height:1.5;">${c.notas_internas}</p></div>`:''}
    <button class="btn btn-outline btn-full" onclick="editarCliente('${c.id}')">Editar datos del cliente</button>
  `;
}
...
function nuevoPedidoCliente() {
  ...
  setTimeout(() => {
    if (estadoApp.clienteActual) {
      const sel = document.getElementById('pedido-cliente');
      if (sel) { sel.value = estadoApp.clienteActual.id; onClienteSeleccionado(); }
    }
  }, 200);
}
...
function selBeneficio(tipo) {
  ...
  ['descuento_pct','bonificacion_kg','ninguno'].forEach(t => {
    const dot = document.getElementById(`rd-${t}`);
    const campo = document.getElementById(`campo-${t}`);
    if (dot) dot.classList.toggle('selected', t===tipo);
...
async function cargarCobros() {
  ...
  if (estadoApp.perfil?.rol==='admin') {
    const { data:comPend } = await sb.from('comisiones').select('monto_comision').eq('retirado',false);
    document.getElementById('cobros-kpi-comision').textContent = formatMoney(comPend?.reduce((s,c)=>s+c.monto_comision,0)||0);
  }

  document.getElementById('cobros-urgentes').innerHTML = (pedSinCobrar||[]).map(p=>`
    <div class="card" style="margin-bottom:8px;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;">
        <div><p style="font-size:13px;font-weight:500;">${p.clientes?.razon_social}</p><p style="font-size:11px;color:#aaa;">Pedido #${String(p.numero||0).padStart(4,'0')}</p></div>
...
async function guardarCobro() {
  ...
  const { data:cobro } = await sb.from('cobros').insert({
    pedido_id:estadoApp.pedidoParaCobro||null,
    cliente_id:estadoApp.clienteActual?.id||null,
    monto, forma_pago:estadoApp.formaPagoActual,
    fecha_cobro:fecha, notas:document.getElementById('cobro-notas').value, rendido:false,
  }).select().single();
  ...
  if (cobro && estadoApp.perfil?.rol==='admin') {
    ...
    await sb.from('comisiones').insert({ cobro_id:cobro.id, pedido_id:estadoApp.pedidoParaCobro||null, cliente_id:estadoApp.clienteActual?.id||null, monto_cobrado:monto, pct_comision:pct, monto_comision:monto*pct/100, retirado:false });
  }
...
async function enviarRecordatorioWa(clienteId) {
  const { data:c } = await sb.from('clientes').select('whatsapp,telefono,razon_social').eq('id',clienteId).single();
  const num = (c?.whatsapp||c?.telefono||'').replace(/\D/g,'');
  if (num) window.open(`https://wa.me/${num}?text=${encodeURIComponent('Hola, le recordamos que tiene un pago pendiente. Muchas gracias. La Cabaña.')}`);
}
    ...
    <div class="card" style="margin-bottom:8px;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;">
        <div>
          ...
          ${d.pedidos?`<p style="font-size:11px;color:#aaa;">Pedido #${String(d.pedidos.numero||0).padStart(4,'0')}</p>`:''}
        </div>
...
async function guardarDocumento() {
  ...
  let clienteId = null;
  if (pedidoId) {
    const { data:p } = await sb.from('pedidos').select('cliente_id').eq('id',pedidoId).single();
    clienteId = p?.cliente_id;
  }
  await sb.from('documentos').insert({
    pedido_id:pedidoId||null, cliente_id:clienteId,
    tipo:estadoApp.tipoDocActual,
    ...
    monto, notas:document.getElementById('doc-notas').value, verificado:false,
  });
...
async function onPedidoDocSeleccionado() {
  const pedidoId = document.getElementById('doc-pedido').value;
  if (!pedidoId) return;
  const { data:p } = await sb.from('pedidos').select('total,subtotal_neto,clientes(razon_social,tipo_iva)').eq('id',pedidoId).single();
  if (!p) return;
...
async function cargarEntregasDia() {
  ...
  const { data:entregas } = await sb.from('entregas').select('id,estado_empresa,estado_cliente,nota_empresa,nota_cliente,pedidos(numero,total,clientes(razon_social,whatsapp,direccion_entrega))').eq('fecha_entrega',hoy);
  const container = document.getElementById('entregas-hoy-container');
  container.innerHTML = (entregas||[]).length ? (entregas||[]).map(e=>`
    <div class="card" style="margin-bottom:8px;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px;">
        <div><p style="font-size:13px;font-weight:500;">${e.pedidos?.clientes?.razon_social}</p><p style="font-size:11px;color:#aaa;">#${String(e.pedidos?.numero||0).padStart(4,'0')} · ${e.pedidos?.clientes?.direccion_entrega||''}</p></div>
        <span class="badge ${e.estado_cliente==='recibido_conforme'?'cobrado':e.estado_cliente==='recibido_con_faltantes'?'sin_cobrar':'en_camion'}">${estadoEntregaLabel(e.estado_cliente)}</span>
      </div>
      ${e.estado_empresa==='pendiente'||e.estado_empresa==='en_camion'?`
        <div style="display:flex;gap:6px;">
      ...
      `:`<div style="background:${e.estado_cliente==='recibido_con_faltantes'?'#fcebeb':'#eaf3de'};border-radius:8px;padding:8px 10px;"><p style="font-size:11px;color:${e.estado_cliente==='recibido_con_faltantes'?'#A32D2D':'#3B6D11'};font-weight:500;">${estadoEntregaLabel(e.estado_cliente)}</p>${e.nota_cliente?`<p style="font-size:11px;color:#888;margin-top:2px;">${e.nota_cliente}</p>`:''}</div>`}
    </div>
...
async function avisarCamionCargado() {
  const hoy = new Date().toISOString().split('T')[0];
  const { data:peds } = await sb.from('pedidos').select('id,clientes(razon_social)').eq('fecha_entrega',hoy).eq('estado','confirmado');
  for (const p of peds||[]) {
    await sb.from('entregas').upsert({pedido_id:p.id,fecha_entrega:hoy,hora_carga:new Date().toISOString(),estado_empresa:'en_camion'},{onConflict:'pedido_id'});
    await sb.from('pedidos').update({estado:'en_camion',updated_at:new Date().toISOString()}).eq('id',p.id);
  }
...
async function registrarEntregaEmpresa(entregaId,estado) {
  ...
  if (estado==='envio_completo') {
    ...
    if (e) await sb.from('pedidos').update({estado:'entregado',updated_at:new Date().toISOString()}).eq('id',e.pedido_id);
  }
...
async function registrarEntregaEmpresaDif(entregaId) {
  ...
  const { data:e } = await sb.from('entregas').select('pedido_id').eq('id',entregaId).single();
  if (e) {
    await sb.from('reclamos').insert({pedido_id:e.pedido_id,tipo:'faltante_empresa',descripcion:nota,estado:'abierto'});
    await sb.from('pedidos').update({estado:'entregado',updated_at:new Date().toISOString()}).eq('id',e.pedido_id);
  }
  ...
  document.getElementById('reclamos-container').innerHTML = (reclamos||[]).map(r=>`
    <div class="card" style="margin-bottom:8px;">
      <div style="display:flex;justify-content:space-between;margin-bottom:8px;">
        <div><p style="font-size:13px;font-weight:500;">${r.clientes?.razon_social||'Sin cliente'}</p><p style="font-size:11px;color:#aaa;">Pedido #${String(r.pedidos?.numero||0).padStart(4,'0')} · ${r.tipo==='faltante_empresa'?'Faltante empresa':'Faltante cliente'}</p></div>
      ...
      ${r.estado==='abierto'?`<div style="display:flex;gap:6px;flex-wrap:wrap;">
...
async function resolverReclamo(id,estado) {
  const res = estado==='cerrado'?prompt('Descripción de la resolución:'):null;
  await sb.from('reclamos').update({estado,resolucion:res,fecha_resolucion:estado==='cerrado'?new Date().toISOString().split('T')[0]:null}).eq('id',id);
...
async function cargarProductos(filtro='todos') {
  ...
  document.getElementById('productos-container').innerHTML = (filtrados||[]).map(p=>{
    ...
    return `
      <div class="card" style="margin-bottom:8px;">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;">
          <div><p style="font-size:13px;font-weight:500;">${p.nombre}</p><p style="font-size:11px;color:#aaa;">${p.categorias?.nombre} · ${p.presentacion||''}</p></div>
          <div style="text-align:right;"><p style="font-size:14px;font-weight:700;font-family:'Playfair Display',serif;">${p.tiene_precio?formatMoney(p.precio_lista):'Sin precio'}</p><p style="font-size:10px;color:#aaa;">la ${p.unidad_venta}</p></div>
        </div>
        <div style="border-top:0.5px solid #f0ede6;padding-top:8px;display:flex;gap:12px;flex-wrap:wrap;">
...
async function cargarRendicion() {
  ...
  if (estadoApp.perfil?.rol==='admin') {
    const { data:comPend } = await sb.from('comisiones').select('monto_comision').eq('retirado',false);
    document.getElementById('rend-comision').textContent = formatMoney(comPend?.reduce((s,c)=>s+c.monto_comision,0)||0);
  }
  ...
  const btn=document.getElementById('btn-rendir');
  if (btn) btn.style.display=cobros?.length?'block':'none';
}
...
async function procesarRendicion() {
  ...
  if (estadoApp.perfil?.rol==='admin') {
    const { data:coms } = await sb.from('comisiones').select('id').eq('retirado',false);
    const comIds = coms?.map(c=>c.id)||[];
    if (comIds.length) await sb.from('comisiones').update({retirado:true,fecha_retiro:fecha}).in('id',comIds);
  }
...
async function cargarComisiones(periodo='mes') {
  ...
  const { data:coms } = await sb.from('comisiones').select('*,clientes(razon_social),pedidos(numero)').gte('created_at',desde).order('created_at',{ascending:false});
  ...
  const porCliente={};
  coms?.forEach(c=>{ const n=c.clientes?.razon_social||'Sin cliente'; if(!porCliente[n])porCliente[n]=0; porCliente[n]+=c.monto_comision; });
  document.getElementById('comisiones-por-cliente').innerHTML=Object.entries(porCliente).sort((a,b)=>b[1]-a[1]).map(([n,m])=>`
    <div class="card" style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
      <p style="font-size:13px;font-weight:500;">${n}</p>
      <p style="font-size:14px;font-weight:700;color:#534AB7;font-family:'Playfair Display',serif;">${formatMoney(m)}</p>
    </div>`).join('');
  document.getElementById('comisiones-detalle').innerHTML=(coms||[]).map(c=>`
    <div class="card" style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
      <div><p style="font-size:12px;font-weight:500;">${c.clientes?.razon_social||'Sin cliente'} — #${String(c.pedidos?.numero||0).padStart(4,'0')}</p><p style="font-size:11px;color:#aaa;">${formatFecha(c.created_at)} · ${c.pct_comision}% de ${formatMoney(c.monto_cobrado)} · ${c.retirado?'Retirado ✓':'Pendiente'}</p></div>
...
async function generarReporte() {
  ...
  document.getElementById('reporte-resumen').innerHTML=`
    <div class="card" style="margin-bottom:10px;">
      <p class="section-title-sm">Resumen ${desde} al ${hasta}</p>
  ...
  document.getElementById('reporte-detalle').innerHTML=(pedidos||[]).map(p=>{
    ...
    return `
      <div class="card" style="margin-bottom:8px;">
        <div style="display:flex;justify-content:space-between;margin-bottom:8px;">
          <div><p style="font-size:13px;font-weight:500;">${p.clientes?.razon_social}</p><p style="font-size:11px;color:#aaa;">Pedido #${String(p.numero||0).padStart(4,'0')} · ${formatFecha(p.fecha_pedido)}</p></div>
          <span class="badge ${p.estado}">${estadoLabel(p.estado)}</span>
        </div>
        <div style="background:#f5f3ef;border-radius:8px;padding:8px 10px;margin-bottom:8px;">
          ${(p.documentos||[]).map(d=>`<p style="font-size:11px;color:#555;">${d.tipo==='factura'?'📄':'📋'} ${d.numero_doc||''} — ${formatMoney(d.monto)}</p>`).join('')}
    ...
    <div class="card" style="margin-bottom:8px;">
      <div style="display:flex;align-items:center;gap:10px;">
        ...
        <div style="flex:1;">
          ...
          <p style="font-size:11px;color:#aaa;">#${String(e.pedidos?.numero||0).padStart(4,'0')} · ${formatMoney(e.pedidos?.total||0)}</p>
        </div>
...
async function cargarConfig() {
  ...
  const el=document.getElementById('config-comision');
  if (config&&el) el.value=config.comision_pct;
  ...
  if (lista) lista.innerHTML=(dists||[]).map(d=>`
    <div style="display:flex;align-items:center;padding:8px 0;border-bottom:0.5px solid #f0ede6;gap:8px;">
      <input type="text" value="${d.nombre}" class="form-input" style="flex:1;" onchange="actualizarDistribuidor('${d.id}',this.value)">
    </div>`).join('');
}

async function guardarConfig() {
  const pct=parseFloat(document.getElementById('config-comision').value);
  await sb.from('configuracion').update({comision_pct:pct,updated_at:new Date().toISOString()}).eq('id',1);
...
async function agregarDistribuidor() {
  ...
  if (!nombre) return;
  await sb.from('distribuidores').insert({nombre});
  await cargarConfig();
}
...
async function cargarFormPrecios() {
  ...
  document.getElementById('precios-form-container').innerHTML=(prods||[]).map(p=>`
    <div class="card" style="margin-bottom:6px;">
      <div style="display:flex;justify-content:space-between;align-items:center;">
        <div><p style="font-size:13px;font-weight:500;">${p.nombre}</p><p style="font-size:11px;color:#aaa;">${p.categorias?.nombre} · Actual: ${p.tiene_precio?formatMoney(p.precio_lista):'Sin precio'}</p></div>
        <input type="number" class="form-input" style="width:120px;" placeholder="Nuevo" id="precio-${p.id}">
      </div>
    </div>`).join('');
}
...
async function guardarListaPrecios() {
  ...
  for (const p of prods||[]) {
    ...
    if (input?.value) {
      ...
      if (p.precio_lista) await sb.from('historial_precios').insert({producto_id:p.id,precio_lista:p.precio_lista,fecha_desde:fecha});
      updates.push(sb.from('productos').update({precio_lista:nuevoPrecio,tiene_precio:true}).eq('id',p.id));
    }
  }
...
async function cargarPortalCliente() {
  ...
  if (!cli) { document.getElementById('portal-pedidos').innerHTML='<p style="font-size:13px;color:#aaa;">No hay datos de cliente asociados a esta cuenta.</p>'; return; }

  const hoy=new Date();
  const inicioMes=new Date(hoy.getFullYear(),hoy.getMonth(),1).toISOString().split('T')[0];

  if (cli.objetivo_kg_mensual) {
    const { data:pedsCli } = await sb.from('pedidos').select('id').eq('cliente_id',cli.id).gte('fecha_pedido',inicioMes).neq('estado','cancelado');
    let kgMes=0;
    if (pedsCli?.length) {
      const { data:items } = await sb.from('pedido_items').select('cantidad').in('pedido_id',pedsCli.map(p=>p.id));
      kgMes=items?.reduce((s,i)=>s+(i.cantidad||0),0)||0;
    ...
    const fondos={ok:'#eaf3de',warning:'#faeeda',danger:'#fcebeb'};
    const textos={ok:'#3B6D11',warning:'#854F0B',danger:'#A32D2D'};
    document.getElementById('portal-objetivo-section').innerHTML=`
      <div style="background:${fondos[clase]};border-radius:14px;padding:12px 14px;margin-bottom:14px;">
        <div style="display:flex;justify-content:space-between;margin-bottom:6px;">
          <p style="font-size:12px;font-weight:500;color:${textos[clase]};">Objetivo: ${cli.objetivo_kg_mensual.toLocaleString()} kg/mes</p>
          <p style="font-size:12px;font-weight:500;color:${textos[clase]};">${Math.round(kgMes).toLocaleString()} kg</p>
        </div>
        ...
        <p style="font-size:10px;color:${textos[clase]};margin-top:6px;">${msgs[clase]}</p>
      </div>`;
  }
  ...
  document.getElementById('portal-pedidos').innerHTML=(pedidos||[]).map(p=>`
    <div class="card" style="margin-bottom:8px;">
      <div style="display:flex;justify-content:space-between;margin-bottom:8px;">
        <div><p style="font-size:13px;font-weight:500;">Pedido #${String(p.numero||0).padStart(4,'0')}</p><p style="font-size:11px;color:#aaa;">${formatFecha(p.fecha_pedido)}</p></div>
        <span class="badge ${p.estado}">${estadoLabel(p.estado)}</span>
      </div>
    ...
    </div>`).join('')||'<p style="font-size:13px;color:#aaa;">Sin pedidos</p>';
  ...
  document.getElementById('portal-facturas').innerHTML=(docs||[]).map(d=>`
    ...
    </div>`).join('')||'<p style="font-size:13px;color:#aaa;">Sin facturas</p>';
  ...
  document.getElementById('portal-pagos').innerHTML=(cobros||[]).map(c=>`
    ...
    </div>`).join('')||'<p style="font-size:13px;color:#aaa;">Sin pagos registrados</p>';
}
...
async function reportarFaltante(pedidoId) {
  const desc=prompt('Describí qué faltó:');
  if (desc===null) return;
  const { data:cli } = await sb.from('clientes').select('id').eq('user_id',estadoApp.usuario.id).single();
...
function abrirModal(titulo, contenido, acciones=[]) {
  ...
  document.getElementById('modal-acciones').innerHTML = acciones.map((a,i) =>
    `<button class="btn ${a.style||'btn-outline'}" id="modal-btn-${i}">${a.label}</button>`
  ).join('');
...
async function logActividad(accion,tabla,registroId=null) {
  try { await sb.from('log_actividad').insert({user_id:estadoApp.usuario?.id,accion,tabla,registro_id:registroId}); } catch(e){}
}
